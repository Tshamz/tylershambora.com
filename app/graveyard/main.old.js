...
==========================================================================*/
  ...
  $.when(
    ...
    $.get('js/gallery-infos.json', function(data) {
      galleryInfos = data;
    })
  ).then(function() {
    initialLoad();
...
==========================================================================*/
  ...
  $(window).on('hashchange', function() {
    ...
    ga('send', 'event', 'Navigation', 'Click', 'hashchange');
    ...
    if ($.inArray(newHash, subGalleries) >= 0) {
      ...
      setTimeout(function() {
        loadPageContent(newHash, function() {
          ...
          ga('send', 'event', 'Navigation', 'Event', '#' + newHash);
          ga('send', 'event', 'Navigation', 'Event', 'Portfolio => Sub-Gallery');
        });
      ...
      setTimeout(function() {
        loadPageContent(newHash, function() {
          ...
          setTimeout(function(){
            ...
            $('html, body').animate({ scrollTop: 0 }, 400);
            ga('send', 'event', 'Navigation', 'Event', '#' + newHash);
            ga('send', 'event', 'Navigation', 'Event', 'Generic Navigation');
